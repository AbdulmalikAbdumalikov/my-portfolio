...
export default function ProjectCarousel({ items, statusLabel }) {
  ...
  return <section className="project-gallery project-directory">
    ...
    <div className="project-list">{filteredItems.map(item => <article className="project-list-card" key={`${item.projectType}-${item.title}`}><ProjectArtwork item={item} index={items.indexOf(item)} /><ProjectCopy item={item} statusLabel={statusLabel} /><span className={`project-type type-${item.projectType}`}>{typeLabels[item.projectType]}</span></article>)}{filteredItems.length % 2 !== 0 && <article className="project-list-card project-list-card--future" aria-label="Yangi loyihalar tayyorlanmoqda"><div className="future-project-art" aria-hidden="true"><i /><i /><b>+</b><small>2026</small></div><div className="future-project-copy"><span>DEV / NEXT</span><h3>Yangiliklar qilishda davom etamiz.</h3><p>Yangi foydali loyihalar va tajribalar ustida ishlash davom etmoqda.</p><em>IN PROGRESS</em></div></article>}{filteredItems.length === 0 && <p className="project-list-empty">Bu turdagi loyiha topilmadi.</p>}</div>
  </section>
}
